...
async function findMine(id){
    const result = await Promise.all([
        ...
        db('comments as c')
            ...
            .leftJoin('tickets_solutions_videos as sv', 't.id', 'sv.ticket_id')
            .select('c.*', 'dv.url as open_video', 'sv.url as resolved_video', 'rt.solution as solution', 'rt.solution_comment_id', 'rt.solution_reply_id',
            db.raw(`CASE 
...
async function findTicketComments(ticket_id){
    const comments = await db('tickets_comments as tc')
        ...
        .select('c.*', 'u.id as author_id', 'u.name as author_name', 'p.url as author_image', db.raw("TRUE as collapsed"))
        .orderBy('tc.id', 'asc');
...
async function findCommentReplies(comment_id){
    const replies = await db('comments_replies as cr')
        ...
        .join('profile_pictures as p', 'p.user_id', 'u.id')
        .select('cr.*', 'u.id as author_id', 'u.name as author_name', 'p.url as author_image')
        .orderBy('cr.id', 'asc');
